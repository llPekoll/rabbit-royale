...
## RABBIT ROYALE — LE CHIEN DE GARDE DU PING.
##
## L'amont lit pingInterval et pingTimeout du handshake (_on_open) et ne s'en
## sert jamais : c'est marqué "connection timeout ❌" dans son README. Sur un
## bureau ça ne se voit pas, une coupure ferme proprement la socket et
## STATE_CLOSED arrive. Sur un téléphone non : dans un tunnel, dans un
## ascenseur, au passage 4G→wifi, le TCP meurt sans FIN. Le pair reste à
## STATE_OPEN, plus un octet n'arrive, et le jeu attend indéfiniment un serveur
## qui ne parle plus.
##
## Le protocole dit quoi faire : si aucun PING n'arrive en pingInterval +
## pingTimeout, la connexion est morte. Ce compteur est réarmé à chaque PING
## reçu (_on_ping) et, à l'échéance, ferme pour que la reconnexion prenne le
## relais.
var _ping_deadline_ms: int = 0
...
func _process(_delta):
	...
	if _socket_state == WebSocketPeer.STATE_OPEN:
		...
		# RABBIT ROYALE : le serveur s'est-il tu ? Voir _ping_deadline_ms. La
		# socket est encore STATE_OPEN — c'est bien le problème — donc seule
		# cette échéance peut trancher, et on ferme pour de bon afin que
		# STATE_CLOSED tombe au tour suivant et déclenche la reconnexion.
		if _ping_deadline_ms > 0 and Time.get_ticks_msec() > _ping_deadline_ms:
			_ping_deadline_ms = 0
			push_warning("[engineio] serveur muet depuis %d ms, fermeture" % (_ping_interval + _pong_timeout))
			_websocket.close()
...
func _on_open(body: String = ""):
	...
	# RABBIT ROYALE : armé dès l'ouverture, pas au premier PING — sinon une
	# connexion qui meurt AVANT le premier ping n'est jamais détectée.
	_arm_ping_deadline()
...
func _on_ping():
	# RABBIT ROYALE : le serveur a parlé, donc il est vivant. On repousse
	# l'échéance avant de répondre — voir _ping_deadline_ms.
	_arm_ping_deadline()
...
## RABBIT ROYALE : repousse l'échéance du chien de garde.
##
## La marge vient du serveur lui-même (pingInterval + pingTimeout, soit 45 s
## ici) plutôt que d'une constante à nous : c'est lui qui décide de sa cadence,
## et une valeur en dur se désaccorderait le jour où il la change.
func _arm_ping_deadline():
	if _ping_interval <= 0:
		_ping_deadline_ms = 0
		return
	_ping_deadline_ms = Time.get_ticks_msec() + _ping_interval + _pong_timeout
