...
def main() -> None:
    ...
    # How long the artist drew the cycle to run, carried through from Aseprite.
    #
    # The growth animation's LENGTH is an art decision, made in the same file
    # the frames were drawn in — so it travels with the frames instead of being
    # re-typed as a constant in the renderer. Re-timing the cycle in Aseprite and
    # re-exporting is then enough; nobody has to remember that a number in
    # garden-growth.ts also has to move, and the sprite sheet cannot end up
    # playing at a speed it was not drawn for.
    hold_ms = [f.get('duration', 100) for f in atlas['frames']]
