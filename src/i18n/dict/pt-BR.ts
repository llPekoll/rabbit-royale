...
export const ptBR: Dict = {
  ...
  run: {
    ...
    homeRaid: 'Voltar \u00b7 d\u00e1 para saquear',
  ...
  energyPanel: {
    ...
    title: 'ENERGIA',
    status: (energy, max, regen) => `${energy}/${max} \u00b7 +${regen} por hora`,
    full: 'Cheia. Nada volta at\u00e9 voc\u00ea gastar.',
    fullIn: (wait) => `Cheia de novo em ${wait}, sozinha.`,
    islandYes: (cost) => `Uma ilha: sim. A travessia leva ${cost}, depois cada escava\u00e7\u00e3o leva 1.`,
    islandNo: (floor) => `Uma ilha: ainda n\u00e3o. Pede ${floor} no tanque.`,
    raidYes: (toll, stake) => `Um saque: sim. ${toll} para entrar, no m\u00e1ximo ${stake}; chegue ao campo e seus passos voltam.`,
    raidNo: (floor) => `Um saque: ainda n\u00e3o. Pede ${floor} no tanque.`,
    dig: (dig, bomb) => `Escavar leva ${dig}, uma bomba ${bomb}. Um X certo devolve, um errado custa 15.`,
    homeRaid: (floor) => `Ainda d\u00e1 para um saque (${floor}). O que voc\u00ea leva para casa fica no tanque.`,
    homeNoRaid: (floor) => `Abaixo de ${floor}: n\u00e3o d\u00e1 mais para saquear. O que voc\u00ea leva para casa fica no tanque.`,
    level: (level, regen, next) => `Toca n\u00edvel ${level} recarrega ${regen} por hora; o n\u00edvel ${level + 1} recarregaria ${next}.`,
    levelMax: (level, regen) => `Toca n\u00edvel ${level} recarrega ${regen} por hora.`,
  },
  islandPick: {
