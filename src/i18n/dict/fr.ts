...
export const fr: Dict = {
  ...
  loop: {
    ...
    raidNeeds: (floor, have, wait) => `Un raid demande ${floor} d\u2019\u00e9nergie. ${have} pour l\u2019instant, assez dans ${wait}.`,
    aMoment: 'un instant',
  ...
  run: {
    ...
    homeRaid: 'Rentrer \u00b7 de quoi piller',
  ...
  recap: {
    ...
    raidLeft: (energy) => `⚡ ${energy} au terrier : de quoi piller`,
  ...
  energyPanel: {
    ...
    title: '\u00c9NERGIE',
    status: (energy, max, regen) => `${energy}/${max} \u00b7 +${regen} par heure`,
    full: 'Plein. Rien ne revient tant que tu n\u2019en d\u00e9penses pas.',
    fullIn: (wait) => `Plein dans ${wait}, tout seul.`,
    islandYes: (cost) => `Une \u00eele : oui. La travers\u00e9e prend ${cost}, puis chaque fouille 1.`,
    islandNo: (floor) => `Une \u00eele : pas encore. Il faut ${floor} en r\u00e9serve.`,
    raidYes: (toll, stake) => `Un raid : oui. ${toll} pour entrer, ${stake} au plus ; atteins le champ et tes pas reviennent.`,
    raidNo: (floor) => `Un raid : pas encore. Il faut ${floor} en r\u00e9serve.`,
    dig: (dig, bomb) => `Une fouille prend ${dig}, une bombe ${bomb}. Un X juste en rend, un X faux en co\u00fbte 15.`,
    homeRaid: (floor) => `Tu as encore de quoi piller (${floor}). Ce que tu ram\u00e8nes reste dans le r\u00e9servoir.`,
    homeNoRaid: (floor) => `Sous ${floor} : plus de quoi piller. Ce que tu ram\u00e8nes reste dans le r\u00e9servoir.`,
    level: (level, regen, next) => `Le terrier niveau ${level} recharge ${regen} par heure ; le niveau ${level + 1} rechargerait ${next}.`,
    levelMax: (level, regen) => `Le terrier niveau ${level} recharge ${regen} par heure.`,
  },
  islandPick: {
  ...
  profile: {
    ...
    damage: (n) => `${n} dég.`,
    shovedIn: 'poussé à l’eau',
    struckDown: 'foudroyé',
    youShoved: (name) => `Tu as poussé ${name} à l’eau`,
