...
export const fr: Dict = {
  ...
  energyPanel: {
    ...
    title: '\u00c9NERGIE',
    perHour: (regen) => `+${regen} par heure`,
    full: 'pleine',
    fullIn: (wait) => `pleine dans ${wait}`,
    yes: 'OUI',
    inWait: (wait) => `dans ${wait}`,
    tickIsland: '\u00eele',
    tickRaid: 'raid',
    island: 'UNE \u00ceLE',
    islandCost: (cross) => `${cross} pour traverser, puis 1 par fouille`,
    raid: 'UN RAID',
    raidCost: (toll, stake) => `${toll} pour entrer, ${stake} au plus. Atteins le champ : tes pas reviennent.`,
    dig: 'CREUSER',
    digCost: (bomb) => `1 par fouille, une bombe ${bomb}. Un X juste en rend, un X faux en co\u00fbte 15.`,
    home: 'RENTRER',
    homeCost: (floor) => `Ce qui reste reste dans le r\u00e9servoir. Au-dessus de ${floor}, tu rentres avec un raid.`,
    homeYes: 'DE QUOI PILLER',
    homeNo: 'PAS DE RAID',
    level: (level, regen, next) => `Terrier niv. ${level} : +${regen}/h \u00b7 niv. ${level + 1} : +${next}/h`,
    levelMax: (level, regen) => `Terrier niv. ${level} : +${regen}/h`,
  },
  islandPick: {
    ...
    fresh: 'Personne dessus encore \u00b7 la longue course',
    shortSafe: 'courte et s\u00fbre',
    join: 'Rejoindre',
    open: 'Ouvrir',
    locked: (need, have) => `S\u2019ouvre \u00e0 ${need} creus\u00e9es \u00b7 toi ${have}`,
    lockedAria: (name, have, need) => `${name} : ${have} carottes creus\u00e9es sur ${need}`,
    lockedShort: 'Ferm\u00e9e',
    tank: (energy, cost) => `\u26a1 ${energy} en r\u00e9serve \u00b7 la travers\u00e9e prend ${cost}`,
