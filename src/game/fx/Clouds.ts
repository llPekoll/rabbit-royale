 ...
 * paid for differently now: the bands sit off the TOP edge, so what a cloud
 * crosses is the sky rather than the tiles a player is reading.
 *
 * There used to be a matching pair of bands hugging the bottom edge. They never
 * read as weather: a cloud is something you look UP at, and a puff creeping in
 * from under the board looked like a rendering fault rather than a sky. Only
 * the top pair survives — see `place` for where each one sits.
 */
 ...
 *
 * Both bands drift HORIZONTALLY: weather moves across a sky, and clouds that
 * slid up and down the sides read as a lift rather than as wind. The bands
 * differ in the height they sit at, not in the direction they travel.
 */
export type CloudBand = 'top' | 'upper';
...
export class CloudField {
  ...
  constructor(parent: Container, private opts: CloudFieldOptions) {
    ...
    const bands: CloudBand[] = ['top', 'upper'];
   ...
   *
   * Each band runs left-to-right or right-to-left; only the height differs.
   * Alternating the direction between the two keeps the sky from reading as one
   * conveyor belt.
   */
  private place(cloud: Cloud, t: number): void {
    ...
    switch (cloud.band) {
      // The bands sit mostly OFF the frame's top edge: a cloud shows its lower
      // edge and hangs the rest into the margin. These sprites are larger
  ...
  update(deltaMs: number): void {
    ...
    for (const cloud of this.clouds) {
      ...
      // Horizontal, always: `top` drifts right, `upper` drifts left.
      const rightwards = cloud.band === 'top';
      if (rightwards) {
        s.x += d;
   ...
   * landscape and then turned to portrait keeps parking its bands against the
   * OLD height. The scenes already re-solve their camera on resize; the sky has
   * to be re-solved with it.
   *
   ...
   *
   * The bands are deliberately parked just off the canvas's top edge (see
   * `place`) — a cloud shows its lower edge and hangs the rest into the margin.
   * That is a statement about the frame, so when the burrow's camera pulls back
   * it drags the whole parked bank into view and the sky suddenly reads as fog
   * rolling over the garden. Counter-scaling keeps the clouds where they were composed
